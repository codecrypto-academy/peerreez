...
export interface BesuNetworkOptions {
    networkName: string;
    subnet: string;
    image: string;
    nodes: BesuNodeConfig[];
    extraRpcNodes?: BesuNodeConfig[];
    mnemonic?: string;
    baseDir: string;
...
export class BesuDeployer {
    docker: Docker;
    options: BesuNetworkOptions;

    constructor(options: BesuNetworkOptions) {
    ...
    async verifyRequirements() {
        ...
        await new Promise<void>((resolve, reject) => {
            exec('node -v', (err: any) => {
                if (err) reject(new Error('Node.js no está disponible.'));
                else resolve();
            });
        });
    ...
    async generateNodeKeys(node: BesuNodeConfig, ipOverride?: string) {
        ...
        return new Promise<void>((resolve, reject) => {
            exec(
                `node ${opPath} create-keys ${ip}`,
                { cwd: nodeDir },
                (err: any, stdout: any, stderr: any) => {
                    if (err) reject(new Error(`Error generando claves para ${node.name}: ${stderr}`));
                    else resolve();
                }
            );
        });
    ...
    async generateAccountKey() {
        ...
        return new Promise<void>((resolve, reject) => {
            exec(
                `node ${opPath} create-keys 172.30.0.30`,
                { cwd: dir },
                (err: any, stdout: any, stderr: any) => {
                    if (err) reject(new Error(`Error generando claves para account: ${stderr}`));
                    else resolve();
                }
            );
        });
    ...
    async generateExtraRpcKeys() {
        ...
        if (!extraRpcNodes || extraRpcNodes.length === 0) return;
        const { join } = await import('path');
        const { exec } = await import('child_process');
        const opPath = join(baseDir, 'src', 'scripts', 'operations.mjs');
        for (const node of extraRpcNodes) {
            ...
            await new Promise<void>((resolve, reject) => {
                exec(
                    `node ${opPath} create-keys ${node.ip}`,
                    { cwd: nodeDir },
                    (err: any, stdout: any, stderr: any) => {
                        if (err) reject(new Error(`Error generando claves para rpc${node.rpcPort}: ${stderr}`));
                        else resolve();
                    }
                );
            });
    ...
    async createExtraRpcConfigs(bootEnode: string) {
    ...
    async createConfigFiles() {
        ...
        if (!bootNode || !minerNode) throw new Error('Faltan nodos bootnode o miner');
    ...
    async launchContainer(node: BesuNodeConfig) {
    ...
    async verifyBootnode() {
        ...
        const bootNode = nodes.find(n => n.type === 'bootnode');
        if (!bootNode) throw new Error('No se encontró el bootnode');
    ...
    async fundMnemonicAccounts() {
        if (!this.options.mnemonic) return;
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            child.on('close', (code: number) => {
                if (code === 0) resolve();
                else reject(new Error(`Error transfiriendo fondos: proceso terminó con código ${code}`));
            });
        });
    }
