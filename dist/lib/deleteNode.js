...
export const deleteNode = async (networkName, nodeName) => {
    ...
    try {
        const containerIdsRaw = execSync(`docker ps -aq --filter "name=^${nodeName}$"`)
            .toString()
            .trim();
            ...
            for (const id of containerIds) {
                ...
                while (!removed && attempts < 5) {
                    try {
                        ...
                        attempts++;
                        console.log(`⚠️ Retry removing container ${id} (${attempts}/5)...`);
                        await new Promise((r) => setTimeout(r, 2000));
                    }
                }
                if (!removed)
                    console.warn(`❌ Could not remove container ${id}`);
            }
        }
        else {
