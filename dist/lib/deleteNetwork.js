...
export const deleteNetwork = async (networkName) => {
    if (!networkName)
        throw new Error("❌ Network name is required");
    const networksDir = path.join(process.cwd(), "src", "lib", "networks", networkName);
    // 1. Eliminar contenedores asociados a la red
    try {
        const containers = execSync(`docker ps -aq --filter "label=network=${networkName}"`).toString().trim();
        if (containers) {
    ...
    try {
        const networks = execSync(`docker network ls --filter "name=${networkName}" --format "{{.Name}}"`).toString().trim();
        if (networks) {
