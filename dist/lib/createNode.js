...
export const createNode = async (params) => {
    ...
    if (!nodeIp)
        throw new Error(`❌ No free IP found in network ${networkName}`);
    ...
    while (!nodeReady) {
        try {
            await axios.post(`http://127.0.0.1:${availablePort}`, {
                jsonrpc: "2.0",
                method: "net_version",
                params: [],
                id: 1
    ...
    switch (nodeType) {
        case "rpc": {
            const rpcResp = await axios.post(`http://127.0.0.1:${availablePort}`, {
                jsonrpc: "2.0",
                method: "net_version",
                params: [],
                id: 1
        ...
        case "signer": {
            let signerResp = null;
            try {
                const resp = await axios.post(`http://127.0.0.1:${availablePort}`, {
                    jsonrpc: "2.0",
                    method: "net_version",
                    params: [],
                    id: 1
